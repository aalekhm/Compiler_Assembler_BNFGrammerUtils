...
class GrammerUtils
{
	public:
		...
		static void									processAssembly(std::vector<std::unique_ptr<OpcodeStmt>>& vAsmStmts);
		static void									generateCode(std::vector<std::unique_ptr<OpcodeStmt>>& vAsmStmts);
		static void									printAssembly(std::vector<std::unique_ptr<OpcodeStmt>>& vAsmStmts);
		static void									readAssembly(int8_t* pByteArray, int32_t iArraySize);
	...
	private:
		...
		static std::unique_ptr<ByteArrayOutputStream>	m_pBAOSAsm;
		static std::unique_ptr<ByteArrayInputStream>	m_pBAISAsm;
		...
		static void									populateAvailableRegisterStack();
		static int32_t								getNextAvailableRegister();
	public:
		...
		static std::stack<E_REGISTERSASM>			m_AvailableRegisterStack;
};