...
#define IS_BIT_SET(__operand__, __bit__)	((__operand__ & (int32_t)__bit__) > 0)
...
std::unique_ptr<ByteArrayOutputStream>	GrammerUtils::m_pBAOSAsm;
std::unique_ptr<ByteArrayInputStream>	GrammerUtils::m_pBAISAsm;

std::vector<Tree*>						FunctionInfo::m_vStaticVariables;
HANDLE									GrammerUtils::m_HColor;

std::stack<E_REGISTERSASM>				GrammerUtils::m_AvailableRegisterStack;
...
#define EMIT_ASM_BYTE(__byte__)							m_pBAOSAsm->writeByte((int8_t)__byte__);
#define EMIT_ASM_INT(__int__)							m_pBAOSAsm->writeInt((int32_t)__int__);

#define READ_ASM_BYTE									m_pBAISAsm->readByte();
#define READ_ASM_INT									m_pBAISAsm->readInt();
...
{
	populateAvailableRegisterStack();
	E_REGISTERSASM eReg = (E_REGISTERSASM)getNextAvailableRegister();
	bool b = true;
	/*
...
void GrammerUtils::processAssembly(std::vector<std::unique_ptr<OpcodeStmt>>& vAsmStmts)
{
	printAssembly(vAsmStmts);
	generateCode(vAsmStmts);

	int8_t* pByteArray = m_pBAOSAsm->toByteArray();
	assert(pByteArray != nullptr);
	if(pByteArray != nullptr)
	{
		readAssembly(pByteArray, m_pBAOSAsm->getSize());
		free(pByteArray);
	}
}

void GrammerUtils::printAssembly(std::vector<std::unique_ptr<OpcodeStmt>>& vAsmStmts)
{
	for (auto& pAsmStmt : vAsmStmts)
	{
		std::cout << (int32_t)pAsmStmt->m_eOpcode << "\t" << opCodeToString(pAsmStmt->m_eOpcode) << "\t";
		for (int32_t i = 0; i < (int32_t)E_OPERAND_NUM::_MAX; i++)
		{
			StmtOperand& pStmtOperand = pAsmStmt->m_Operands[i];

			if (i > 0 && pStmtOperand.m_eOperandType != E_OPERAND_TYPE::_NONE)
			{
				std::cout << ", ";
			}

			switch (pStmtOperand.m_eOperandType)
			{
				case E_OPERAND_TYPE::_CONST:
				case E_OPERAND_TYPE::_LABEL:
				case E_OPERAND_TYPE::_REG:
				{
					std::cout << pStmtOperand.m_sOperandData;
				}
				break;
				case E_OPERAND_TYPE::_MEM:
				{
					int32_t iMemoryBit = pStmtOperand.m_eMemoryOperandBit;

					std::cout << "[";
					if (IS_BIT_SET(iMemoryBit, E_MEMOP::BASE_REG))
					{
						std::cout << pStmtOperand.m_sMemoryOperandDataMap[E_MEMOP::BASE_REG];
					}

					if (IS_BIT_SET(iMemoryBit, E_MEMOP::INDEX_REG))
					{
						std::cout << " + " << pStmtOperand.m_sMemoryOperandDataMap[E_MEMOP::INDEX_REG];
					}

					if (IS_BIT_SET(iMemoryBit, E_MEMOP::SCALE))
					{
						std::cout << " * " << pStmtOperand.m_sMemoryOperandDataMap[E_MEMOP::SCALE];
					}

					if (IS_BIT_SET(iMemoryBit, E_MEMOP::DISP_ADD_SUB))
					{
						std::cout << " " << pStmtOperand.m_sMemoryOperandDataMap[E_MEMOP::DISP_ADD_SUB];

						if (IS_BIT_SET(iMemoryBit, E_MEMOP::DISP_CONST))
						{
							std::cout << " " << pStmtOperand.m_sMemoryOperandDataMap[E_MEMOP::DISP_CONST];
						}
					}
...
void GrammerUtils::generateCode(std::vector<std::unique_ptr<OpcodeStmt>>& vAsmStmts)
{
	m_pBAOSAsm = std::make_unique<ByteArrayOutputStream>();
	for (auto& pAsmStmt : vAsmStmts)
	{
		// iEncodedOpCode:
		// 3		 2		   1		 0
		...
		//			 |--| |--| |--| |--| |-------|
		//			  |		|	|	 |		 |___________> Opcode
		//			  |		|	|	 |___________________> Operand Count
		//			  |		|	|________________________> 1st Operand Type
		//			  |		|____________________________> 2nd Operand Type
		//			  |__________________________________> 3rd Operand Type

		int32_t iEncodedOpCode = 0;
		iEncodedOpCode |= (((int32_t)pAsmStmt->m_Operands[2].m_eOperandType & 0xF) << 20);
		iEncodedOpCode |= (((int32_t)pAsmStmt->m_Operands[1].m_eOperandType & 0xF) << 16);
		iEncodedOpCode |= (((int32_t)pAsmStmt->m_Operands[0].m_eOperandType & 0xF) << 12);

		iEncodedOpCode |= (((int32_t)pAsmStmt->m_iOperandCount & 0xF) << 8);
		iEncodedOpCode |= ((int32_t)pAsmStmt->m_eOpcode & 0xFF);

		std::cout	<< opCodeToString(pAsmStmt->m_eOpcode) << "\t" \
					<< iEncodedOpCode << "\t" \
					<< ((int32_t)pAsmStmt->m_Operands[2].m_eOperandType & 0xF) << " " \
					<< ((int32_t)pAsmStmt->m_Operands[1].m_eOperandType & 0xF) << " " \
					<< ((int32_t)pAsmStmt->m_Operands[0].m_eOperandType & 0xF) << " " \
					<< ((int32_t)pAsmStmt->m_iOperandCount & 0xF) << " " \
					<< ((int32_t)pAsmStmt->m_eOpcode & 0xFF) << "\n";

		EMIT_ASM_INT(iEncodedOpCode);
		...
		for (int32_t i = 0; i < (int32_t)E_OPERAND_NUM::_MAX; i++)
		{
			...
			switch (pStmtOperand.m_eOperandType)
			{
				case E_OPERAND_TYPE::_CONST:
				{
					EMIT_ASM_INT( atoi(pStmtOperand.m_sOperandData.c_str()) );
				}
				break;
				case E_OPERAND_TYPE::_LABEL:
				{
				...
				case E_OPERAND_TYPE::_REG:
				{
					EMIT_ASM_BYTE( stringToAsmRegister(pStmtOperand.m_sOperandData) );
				}
				break;
				case E_OPERAND_TYPE::_MEM:
				{
					int32_t iMemoryBit = pStmtOperand.m_eMemoryOperandBit;
					EMIT_ASM_INT( iMemoryBit );

					std::string sMemoryData = "";
					if (IS_BIT_SET(iMemoryBit, E_MEMOP::BASE_REG))
					{
						sMemoryData = pStmtOperand.getDataForBit(E_MEMOP::BASE_REG);
						EMIT_ASM_BYTE( stringToAsmRegister(sMemoryData.c_str()) );
					}

					if (IS_BIT_SET(iMemoryBit, E_MEMOP::INDEX_REG))
					{
						sMemoryData = pStmtOperand.getDataForBit(E_MEMOP::INDEX_REG);
						EMIT_ASM_BYTE( stringToAsmRegister(sMemoryData.c_str()) );
					}

					if (IS_BIT_SET(iMemoryBit, E_MEMOP::SCALE))
					{
						sMemoryData = pStmtOperand.getDataForBit(E_MEMOP::SCALE);
						EMIT_ASM_BYTE( atoi(sMemoryData.c_str()) );
					}
					...
					if (IS_BIT_SET(iMemoryBit, E_MEMOP::DISP_ADD_SUB))
					{
						if (IS_BIT_SET(iMemoryBit, E_MEMOP::DISP_CONST))
						{
							sMemoryData = pStmtOperand.getDataForBit(E_MEMOP::DISP_CONST);
							int32_t iDisplacementConst = atoi(sMemoryData.c_str());

							sMemoryData = pStmtOperand.getDataForBit(E_MEMOP::DISP_ADD_SUB);
							if (sMemoryData == "-")
								iDisplacementConst *= -1;

							EMIT_ASM_INT( iDisplacementConst );
						}
					}
				}
				break;
...
void GrammerUtils::readAssembly(int8_t* pByteArray, int32_t iArraySize)
{
	m_pBAISAsm = std::make_unique<ByteArrayInputStream>(pByteArray, iArraySize);

	E_OPCODEASM eOpCode = E_OPCODEASM::NONE;
	int8_t iOperandCount = 0;
	E_OPERAND_TYPE eOperandType[3];
	int32_t iIntOperand = 0;
	int8_t iByteOperand = 0;
	while (NOT m_pBAISAsm->isEOF())
	{
		int32_t iEncodedOpCode = m_pBAISAsm->readInt();

		eOpCode = (E_OPCODEASM)(iEncodedOpCode & 0xFF);
		iOperandCount = ((iEncodedOpCode >> 8) & 0xF);
		eOperandType[0] = (E_OPERAND_TYPE)((iEncodedOpCode >> 12) & 0xF);
		eOperandType[1] = (E_OPERAND_TYPE)((iEncodedOpCode >> 16) & 0xF);
		eOperandType[2] = (E_OPERAND_TYPE)((iEncodedOpCode >> 20) & 0xF);

		std::cout << iEncodedOpCode << "\t" << opCodeToString((E_OPCODEASM)eOpCode);
		if (iOperandCount > 0)
		{
			for (int i = 0; i < iOperandCount; i++)
			{
				switch (eOperandType[i])
				{
					case E_OPERAND_TYPE::_CONST:
					{
						iIntOperand = READ_ASM_INT;

						std::cout << " " << iIntOperand;
					}
					break;
					case E_OPERAND_TYPE::_LABEL:
					{
					...
					case E_OPERAND_TYPE::_REG:
					{
						iByteOperand = READ_ASM_BYTE;
						std::cout << " " << static_cast<int16_t>(iByteOperand);
					}
					break;
					case E_OPERAND_TYPE::_MEM:
					{
						...
						std::string sMemoryData = "";
						if (IS_BIT_SET(iIntOperand, E_MEMOP::BASE_REG))
						{
							iByteOperand = READ_ASM_BYTE;
							std::cout << " " << static_cast<int16_t>(iByteOperand);
						}

						if (IS_BIT_SET(iIntOperand, E_MEMOP::INDEX_REG))
						{
							iByteOperand = READ_ASM_BYTE;
							std::cout << " " << static_cast<int16_t>(iByteOperand);
						}

						if (IS_BIT_SET(iIntOperand, E_MEMOP::SCALE))
						{
							iByteOperand = READ_ASM_BYTE;
							std::cout << " " << static_cast<int16_t>(iByteOperand);
						}

						if (IS_BIT_SET(iIntOperand, E_MEMOP::DISP_ADD_SUB))
						{
							if (IS_BIT_SET(iIntOperand, E_MEMOP::DISP_CONST))
							{
								int32_t iDisplacement = READ_ASM_INT;
								std::cout << " " << iDisplacement;
							}
						}
					}
					break;
...
{
	Tree* pASTNode = TinyAssemberReader::makeLeaf(eASTNodeType, sText);
	return pASTNode;
}
...
void GrammerUtils::populateAvailableRegisterStack()
{
	for (int32_t iReg = (int32_t)E_REGISTERSASM::RMAX - 1; iReg >= (int32_t)E_REGISTERSASM::R0; iReg--)
	{
		m_AvailableRegisterStack.push((E_REGISTERSASM)iReg);
	}
}

int32_t GrammerUtils::getNextAvailableRegister()
{
	int32_t iReg = -1;
	if (NOT m_AvailableRegisterStack.empty())
	{
		iReg = (int32_t)m_AvailableRegisterStack.top();
		m_AvailableRegisterStack.pop();
	}
