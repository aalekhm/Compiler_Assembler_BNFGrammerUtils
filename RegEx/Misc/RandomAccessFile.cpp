...
void RandomAccessFile::initBufferedReader(size_t iBufferSize)
{
	BufferData bufferData(iBufferSize);
	m_fStream.read((char*)bufferData.get(), iBufferSize);
...
unsigned long RandomAccessFile::read(int8_t* pBuf, int iLength)
{
	...
	//if (m_fStream.good() && NOT isEOF())
	//{
	//	if ((m_iFileOffset + iLength) <= m_iFileSize - 1)
	//	{
	//		m_fStream.read((char*)pBuf, iLength);
	//
	//		size_t iBytesRead = m_fStream.gcount();
...
unsigned long RandomAccessFile::readFully(int8_t* pBuf)
{
	...
	//return read(pBuf, 0, m_iFileSize);
}
...
unsigned long RandomAccessFile::read(int8_t* pBuf, int iOffset, int iLength)
{
	...
	//	if (iOffset >= 0 && iOffset < m_iFileSize)
	//	{
	//		int32_t iMaxReadOffset = (iOffset + 1 + iLength);
	//		if (iMaxReadOffset >= m_iFileSize)
	//		{
	//			iLength = m_iFileSize - (iOffset + 1);
	//		}
	//
	//		m_fStream.seekg(iOffset);
	//		m_fStream.read((char*)pBuf, iLength);
...
uint8_t RandomAccessFile::readByte()
{
	...
	//if ((m_iFileOffset + 1) <= m_iFileSize - 1)
	//{
	//	m_iFileOffset++;
	//	return (m_fStream.get() & 0xff);
	//}
	//
	//return -1;
}
...
uint32_t RandomAccessFile::readInt()
{
	...
	//if ((m_iFileOffset + 3) <= m_iFileSize - 1)
	//{
...
uint64_t RandomAccessFile::readLong()
{
	...
	//if ((m_iFileOffset + 7) <= m_iFileSize - 1)
	//{
...
bool RandomAccessFile::isEOF()
{
	...
	//return (m_iFileOffset == m_iFileSize - 1);
