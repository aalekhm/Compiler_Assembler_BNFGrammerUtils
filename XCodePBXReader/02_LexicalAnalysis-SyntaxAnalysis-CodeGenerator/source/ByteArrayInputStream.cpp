...
int8_t ByteArrayInputStream::readByte()
{
	...
	int8_t iByte = (int8_t)m_pByteArrayRO[m_iCurrentPos++];
	return iByte;
}
...
int16_t ByteArrayInputStream::readShort()
{
	...
	if (m_eEndianness == ENDIANNESS::LITTLE)
	{
		...
		int16_t* pShort = (int16_t*)&m_pByteArrayRO[m_iCurrentPos];
		iShort = *pShort;
...
int32_t ByteArrayInputStream::readInt()
{
	...
	if (m_eEndianness == ENDIANNESS::LITTLE)
	{
		...
		int32_t* pInt = (int32_t*)&m_pByteArrayRO[m_iCurrentPos];
		iInt = *pInt;
...
int64_t ByteArrayInputStream::readLong()
{
	...
	if (m_eEndianness == ENDIANNESS::LITTLE)
	{
		...
		int64_t* pLong = (int64_t*)&m_pByteArrayRO[m_iCurrentPos];
		iLong = *pLong;
...
int8_t ByteArrayInputStream::readByteAtPos(int64_t iOffset)
{
	...
	int8_t iByte = (int8_t)m_pByteArrayRO[iOffset];
	return iByte;
}
...
int16_t	ByteArrayInputStream::readShortAtPos(int64_t iOffset)
{
	...
	if (m_eEndianness == ENDIANNESS::LITTLE)
	{
		...
		int16_t* pShort = (int16_t*)&m_pByteArrayRO[iOffset];
		iShort = *pShort;
	}
	else
	{
...
int32_t	ByteArrayInputStream::readIntAtPos(int64_t iOffset)
{
	...
	if (m_eEndianness == ENDIANNESS::LITTLE)
	{
		...
		int32_t* pInt = (int32_t*)&m_pByteArrayRO[iOffset];
		iInt = *pInt;
	}
	else
	{
...
int64_t	ByteArrayInputStream::readLongAtPos(int64_t iOffset)
{
	...
	if (m_eEndianness == ENDIANNESS::LITTLE)
	{
		...
		int64_t* pLong = (int64_t*)&m_pByteArrayRO[iOffset];
		iLong = *pLong;
	}
	else
	{
...
bool ByteArrayInputStream::isEOF()
{
	return (m_iCurrentPos >= m_iArraySize);
}