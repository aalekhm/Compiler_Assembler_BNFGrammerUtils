...
Token StringTokenizer::readPBXNumber()
{
	initRead();
	bool bHasDot = false;
	...
	return createToken(TokenType_::Type::TK_PBXNUMBER);
}
...
Token StringTokenizer::readAssemblySingleLineComment()
{
	consume(1);
	initRead();
...
Token StringTokenizer::readDefault(char ch0)
{
	...
	else if(isdigit(ch0) || (ch0 == '-' && isdigit(ch1)))					{ return readPBXNumber(); }
	...
	else if(isalpha(ch0) || ch0 == '_' || ch0 == '#' || ch0 == '.' | ch0 == '/')			
																			{ return readPBXIdentifier(); }
...
Token StringTokenizer::readPBXIdentifier()
{
	initRead();
	consume(1);
	...
	{
		char ch = peek(0);
		if (isalpha(ch) || ch == '_' || isdigit(ch) || ch == '.' || ch == '/')
			consume(1);
		else
		if (ch == '(')
		{
	...
	return createToken(TokenType_::Type::TK_PBXIDENTIFIER);
}
